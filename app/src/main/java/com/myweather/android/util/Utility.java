...
public class Utility {
    ...
    public static boolean handleProvinceResponse(String response) {
        if (!TextUtils.isEmpty(response)) {
            try {
                JSONArray allProvinces = new JSONArray(response);//JSON型数组
                for (int i = 0; i < allProvinces.length(); i++) {
                    //循环遍历JSONArray数组，从中取出的每个元素都是一个JSONObject对象，
                    //此对象包含id，name，version等自定义的数据
                    //用JSONObject方法进行解析
    ...
    //返回weather类对象用于WeatherActivity类中requestWeather方法里的response方法中
